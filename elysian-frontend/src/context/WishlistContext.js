...
export function WishlistProvider({ children }) {
    ...
    useEffect(() => {
        const loadWishlistFromServer = async () => {
            ...
            try {
                const response = await ky.get('http://localhost:5000/api/wishlist', { credentials: 'include' }).json();
    ...
    const removeFromWishlist = async (id) => {
        ...
        try {
            await ky.delete(`http://localhost:5000/api/wishlist/${id}`, { credentials: 'include' });
        } catch (error) {
